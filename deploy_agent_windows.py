...
def deploy_sql_migrations():
    ...
    os.system(f"docker exec -it sql-server /opt/mssql-tools18/bin/sqlcmd -S 'localhost,1433;TrustServerCertificate=True' -U sa -P {getSqlPassword()} -d master -i /usr/src/init.sql")
    os.system(f"docker exec -it sql-server /opt/mssql-tools18/bin/sqlcmd -S 'localhost,1433;TrustServerCertificate=True' -U sa -P {getSqlPassword()} -d master -i /usr/src/populate.sql")
